...
DESCRIPTIONS = [
    "Автоугон.",
    "Кража кошелька в общественном транспорте.",
    "Поджог административного здания.",
    "Незаконное проникновение на дачный участок.",
    "Причинение смерти по неосторожности при ДТП.",
    "Незаконное хранение огнестрельного оружия.",
    "Подозрение на мошенничество по телефону.",
    "Угроза убийством с применением холодного оружия.",
    "Умышленное причинение тяжкого вреда здоровью при самообороне.",
    "Поджог государственного имущества.",
    "Хищение государственного имущества.",
    "Кража со взломом.",
    "Разбойное нападение на продуктовый павильон.",
    "Незаконный сбыт огнестрельного оружия.",
    "Взяточничество.",
    "Неумышленное убийство.",
    "Телефонное мошенничество под видом сотрудников банка.",
    "Мошенничество в крупном масштабе.",
    "Сбыт поддельных денежных купюр в торговом центре."
]
...
def generate_random_date():
    ...
    return f"{str(random_date.day) if random_date.day > 9 else "0" + str(random_date.day)} {MONTHS[random_date.month - 1]} {random_date.year}"
...
def main():
    ...
    while True:
        try:
            count_reports = int(input("Введите количество заявлений для генерации (от 1 до 9999): "))
            if 1 <= count_reports <= 9999:
                break
    ...
    count_investigations = count_reports
